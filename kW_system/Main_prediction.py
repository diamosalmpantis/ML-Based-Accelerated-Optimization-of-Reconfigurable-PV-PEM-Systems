# -*- coding: utf-8 -*-
"""
Created on Mon Mar 16 12:06:49 2026
...
# PV parameters (From your original code)
Iph_ref = 7.5    # Reference photo-generated current (A)
...
Prague, Czech Republic → Latitude: 50.1° N, Longitude: 14.4° E
Andorra (Andorra la Vella) → Latitude: 42.5° N, Longitude: 1.5° E'''

latitude = 55.7 #Change this to your location
longitude = 13.2 # Change this to your location
...
def perturb_observe_series(params, irradiance, num_modules_series, num_modules_parallel, voltage_start=0, voltage_step=0.1):
    voltages = []
    currents = []
    powers = []
    resistances = []
...
def optimize_pv_and_pem_configuration(params, irradiance, p_cat, p_an, δ_mem, A_cell, a_an, a_cat, i_0_an, i_0_cat, T_ref, i_lim,
                                      weight_mppt=0.215, weight_StH=0.366, weight_econ=0, storage_capacity=5000):
    ...
    if irradiance < 80:
        ...
        return (
            2,   # num_modules_series
            90,  # num_modules_parallel
            40,  # num_cell_series
            1,   # num_cell_parallel
            0,   # operating_voltage
            0,   # operating_current
    ...
    for T in range(345, 356, 5): 
      ...
      # Iterate through possible configurations
        for num_modules_series in range(2, 3):  # Adjust as needed
            for num_modules_parallel in range(40,91,2):  # Adjust as needed
                ...
                for num_cell_series in range(40, 41):  # Adjust as needed
                    for num_cell_parallel in range(1, 2):  # Adjust as needed
                        # Initialize stored_hydrogen for each configuration iteration
                        stored_hydrogen = 0 # Initialize with a starting value or load from a persistent source if needed
                        ...
                        capex_norm = total_capex/ 123000   # adjust 50000 to realistic max cost
                        hydrogen_production_norm = hydrogen_production / 0.82
                        StH_efficiency_norm = StH_efficiency /  0.168
                        distance_to_mppt_norm = distance_to_mppt / 1000
    ...
    if best_configuration is None:
                    ...
                    num_modules_series, num_modules_parallel, num_cell_series, num_cell_parallel, operating_voltage, operating_current, mppt_voltage, mppt_current,T,hydrogen_production, StH_efficiency = 6,288,100,1,0,0,0,0,335,0,0
...
storage_capacity = 5000  # example in kg or kWh – set according to your case
stored_hydrogen = 0     # persistent across days
...
for day, group in grouped:
    ...
    for entry in irradiance_temperature_mapping:
        ...
        best_configuration = optimize_pv_and_pem_configuration(params_with_temp, irradiance, p_cat, p_an, δ_mem, A_cell, a_an, a_cat, i_0_an, i_0_cat,
                                                               T_ref, i_lim,weight_mppt=0.215, weight_StH=0.366, weight_econ=0,storage_capacity=5000)
        if best_configuration is  None:
            num_modules_series, num_modules_parallel, num_cell_series, num_cell_parallel, operating_voltage, operating_current, mppt_voltage, mppt_current,T,hydrogen_production, StH_efficiency = 8,46,90,1,0,0,0,0,335,0,0
            net_hydrogen = hydrogen_production - hourly_hydrogen_target
            potential_storage = stored_hydrogen + net_hydrogen
            stored_hydrogen = potential_storage
        ...
        if best_configuration is not None and len(best_configuration) == 11:
            ...
            if  mppt_current >  np.max(current_range_pem) or irradiance < 80:
                ...
                hydrogen_production = (operating_voltage * operating_current * production_efficiency) / (LHV)# Calculate hydrogen production

            print(f'In Day {day} and Hour {hour} the hydrogen production is {hydrogen_production} and efficiency {production_efficiency}' )
...
df_flexible.to_excel('Direct_results_multi_ML_Lund_main.xlsx', index=False)
